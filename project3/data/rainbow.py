...
from blake_wrapper import BLAKE #import BLAKE hash function
import random
import string

aux_dict = {} #the table will have all the possible characters of the password as keys and numbers I'll assign them as values
...
def rainbowAttack(rt, hash, chain_len, blake): #using the rainbow table rt, find the password corresponding to hash given
...
def createRainbowTable(rt, chain_num, chain_len, blake):
...
def main(argc, argv):
	...
	password = rainbowAttack(rt, argv[1], chain_len, blake) #execute a rainbow attack using the rainbow table created on the first argument of the script(the hash)
	if(password == -1):
